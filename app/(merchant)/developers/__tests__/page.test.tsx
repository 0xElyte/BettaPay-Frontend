...
describe('DevelopersPage - API Key Creation (#321)', () => {
  ...
  it('calls apiClient.post("/api/keys") and displays full key in success dialog on creation', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('bp_test_fullsecretkey999888777')).toBeInTheDocument();
      expect(screen.getByText(/Please copy your full API key now/i)).toBeInTheDocument();
    });
    ...
    const doneBtn = screen.getByRole('button', { name: /Done \/ I've Saved My Key/i });
    fireEvent.click(doneBtn);

    await waitFor(() => {
      expect(screen.queryByText('bp_test_fullsecretkey999888777')).not.toBeInTheDocument();
    });
